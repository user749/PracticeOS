...
{
    if (command_id < 0 || command_id >= PRACTICEOS_MAX_ISR80H_COMMANDS)
    {
    ...
    {
        panic("Your are attempting to overwrite an existing command\n");
    }
...
void* isr80h_handle_command(int command, struct interrupt_frame* frame)
{
    ...
    if (command < 0 || command >= PRACTICEOS_MAX_ISR80H_COMMANDS)
    {
        //invalid command
        return 0;
    }
